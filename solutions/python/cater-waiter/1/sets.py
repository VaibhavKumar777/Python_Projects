...
def tag_special_ingredients(dish):
    ...
    special = SPECIAL_INGREDIENTS & set(dish_ingredients)
...
def singleton_ingredients(dishes, intersection):
    ...
    all_ingredients = set()       # store every ingredient we see
    repeated = set()              # store ingredients that appear more than once

    for dish in dishes:
        for ingredient in dish:
            if ingredient in all_ingredients:
                repeated.add(ingredient)   # seen again → repeated
            else:
                all_ingredients.add(ingredient)  # first time we see it

    # singletons = all ingredients - repeated ones
    return all_ingredients - repeated
